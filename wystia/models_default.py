...
from typing import Optional, List, Dict, Union, get_type_hints

from .log import LOG
from .utils.parse import get_srt_duration, as_datetime
...
class VideoData:
    ...
    def __init__(self, assets=None, **kwargs):
        ...
        self.status = VideoStatus(kwargs.pop('status', 'not_found'))
        self.created = as_datetime(kwargs.pop('created', None))
        self.updated = as_datetime(kwargs.pop('updated', None))
        ...
        vars = get_type_hints(type(self))
        for var in vars:
            if var in kwargs:
                setattr(self, var, kwargs.get(var))
            elif not hasattr(self, var):
    ...
    def from_dict(cls, d):
        ...
        for attr in ('created', 'updated'):
            val = d.pop(attr)
            if val:
                setattr(obj, attr, as_datetime(val))
...
class UploadResponse:
    ...
    def __init__(self, **kwargs):
        ...
        self.status = VideoStatus(self.status or 'queued')

        # Convert known datetime attributes to :class:`datetime` objects
        try:
            self.created = as_datetime(self.created)
            self.updated = as_datetime(self.updated)
